...
void ft_data::rm_incomplete_samples() //removes samples which have missing point annotations
{
...
write(FileStorage &fs) const
{
	...
	for(int i = 0; i < int(connections.size()); i++){
		...
		sprintf(str,"connections_%d_0",i); ss = str; fs << ss << connections[i][0];
		sprintf(str,"connections_%d_1",i); ss = str; fs << ss << connections[i][1];
	}
	fs << "n_symmetry" << (int)symmetry.size();
	for(int i = 0; i < int(symmetry.size()); i++){
		char str[256]; const char* ss;
		sprintf(str,"symmetry_%d",i); ss = str; fs << ss << symmetry[i];
	}
	fs << "n_images" << (int)imnames.size();
	for(int i = 0; i < int(imnames.size()); i++){
		char str[256]; const char* ss;
		sprintf(str,"image_%d",i); ss = str; fs << ss << imnames[i];
	}
	int n = points[0].size(),N = points.size();
...
read(const FileNode& node)
{
	...
	for(int i = 0; i < n; i++){
		char str[256]; const char* ss;
		sprintf(str,"connections_%d_0",i); ss = str; node[ss] >> connections[i][0];
		sprintf(str,"connections_%d_1",i); ss = str; node[ss] >> connections[i][1];
	}
	node["n_symmetry"] >> n; symmetry.resize(n);
	for(int i = 0; i < n; i++){
		char str[256]; const char* ss;
		sprintf(str,"symmetry_%d",i); ss = str; node[ss] >> symmetry[i];
	}
	node["n_images"] >> n; imnames.resize(n);
	for(int i = 0; i < n; i++){
		char str[256]; const char* ss;
		sprintf(str,"image_%d",i); ss = str; node[ss] >> imnames[i];
	}
